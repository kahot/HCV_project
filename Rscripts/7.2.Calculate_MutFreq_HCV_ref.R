...
##### running the above script will create the following list:
...
#
source("~/programs/HCV/Rscripts/MutationFreqSum.ref.R")


##########################################
#Plot summary of 1) Transistion mutations
TransFiles<-list.files("Output/MutFreq/ref/",pattern="Transition")
TransFiles<-TransFiles[-6]

#total number of samples 
NofSamples<-length(Overview_sum_ref)

for (i in 1:length(TransFiles)){
                mdata<-read.csv(paste0("Output/MutFreq/ref/",TransFiles[i]),stringsAsFactors=FALSE,row.names=1)
                ...
                #pdf(paste0("Output/MutFreq/Fig/",filename,".pdf"), width = 6, height = 15)
                #par(mfcol=c(3,2))
                ...
                if (i==1|i==2){
                       ggsave(filename=paste0("Output/MutFreq/Fig.ref/",filename,".pdf"),width=5, height=7, units='in',device='pdf', plot=MFplot)}
                else{
                        ggsave(filename=paste0("Output/MutFreq/Fig.ref/",filename,".pdf"), width=7, height=7, units='in',device='pdf',plot=MFplot)
                        }
...
Tv1Files<-list.files("Output/MutFreq/ref/",pattern="Transversion_1.csv")
Tv2Files<-list.files("Output/MutFreq/ref/",pattern="Transversion_2.csv")

#modify the date of the file name!
for (i in c(1 ,2,3,4,5,7)){
        mdata1<-read.csv(paste0("Output/MutFreq/ref/",Tv1Files[i]),stringsAsFactors=FALSE,row.names=1)
        mdata2<-read.csv(paste0("Output/MutFreq/ref/",Tv2Files[i]),stringsAsFactors=FALSE,row.names=1)
        mdata1<-mdata1[1:NofSamples,]
        mdata2<-mdata2[1:NofSamples,]
        if (i==1|i==2){
        mdata<-cbind(mdata1[,c(1,4)],mdata2[,c(2,3)])
        mdata<-mdata[,c(1,3,4,2)]}
        else mdata<-mdata1+mdata2
        ...
        MFplot<-ggplot(dat,aes(x=variable,y=value,fill=variable))+geom_boxplot(alpha=0.5)+labs(x="Nucleotide",y="Mutation frequency")+
                ...
                theme(plot.title = element_text(hjust = 0.5))
        ggsave(filename=paste0("Output/MutFreq/Fig.ref/",filename,".pdf"),plot=MFplot,width=7, height=7, units='in',device='pdf')
...
####################################################
### Average mut.freq. of all 4 bases (A&T only for transition mutations) and plot different mutation types together
...
for (i in 1:length(TransFiles)){
        if (i==1|i==2|i==3|i==4){
                mdata<-read.csv(paste0("Output/MutFreq/ref/",TransFiles[i]),stringsAsFactors=FALSE,row.names=1)
                mdata<-mdata[1:NofSamples,1:2]  #use A and T only for nonCpGmaming mutations
                dat<-melt(mdata)
                dat<-dat[!(is.na(dat$value)),]
...
MFplot<-ggplot(Transitions,aes(x=variable,y=value,fill=variable))+geom_boxplot(alpha=0.5)+labs(x="Mutation Type",y="Mutation frequency")+
        ...
        theme(plot.title = element_text(hjust = 0.5))
ggsave(filename=paste0("Output/MutFreq/Fig.ref/",title,"_Summary(A,T only).pdf"),width=9, height=7, units='in',plot=MFplot)
...
Tv1Files<-list.files("Output/MutFreq/ref/",pattern="Transversion_1")
Tv2Files<-list.files("Output/MutFreq/ref/",pattern="Transversion_2")
Tv1Files<-Tv1Files[-6]
Tv2Files<-Tv2Files[-6]
...
MFplot2<-ggplot(Transvs,aes(x=variable,y=value,fill=variable))+geom_boxplot(alpha=0.5)+labs(x="Mutation Type",y="Mutation frequency")+
        ...
        ggtitle(paste0(title)) + theme(plot.title = element_text(lineheight=3, face="bold", color="black", size=13))+
        theme(plot.title = element_text(hjust = 0.5))
ggsave(filename=paste0("Output/MutFreq/Fig.ref/",title,"_Summary.pdf"),width=9, height=7, units='in',plot=MFplot2)
...
##############
# Make Frequncy Histograms  (like SelCoeff)
...
for (i in 1:length(Overview_sum_ref)){
        OverviewData<-Overview_sum_ref[[i]]
        filename<-paste0("Output/MutFreq/",names(Overview_sum_ref)[i],"_MutFreq.pdf")
        ...
        for (typeofsite in c("syn", "nonsyn")){
                ...
                for (wtnt in c("a", "t", "c", "g")){
                        datavector<- dat$freq.Ts.ref[dat$Type==typeofsite & dat$ref==wtnt]
                        ...
                        #datavectorNonCpG<- dat$EstSelCoeff[dat$Type==typeofsite & dat$ref==wtnt & dat$makesCpG==0]
...
for (i in 1:length(Overview_summary)){
        ...
        for (typeofsite in c("syn", "nonsyn")){
                ...
                for (wtnt in c("a", "t")){
                        ...
                        datavectorCpG<- dat$freq.Ts.ref[dat$Type==typeofsite & dat$ref==wtnt & dat$makesCpG==1]
...
for (i in 1:length(Overview_summary)){
        ...
        for (typeofsite in c("syn", "nonsyn")){
                ...
                mycol <- rgb(190, 190, 190, max = 255, alpha = 100, names = "grey50")
                for (wtnt in c("a", "t")){
                        datavectorNonCpG<- dat$freq.Ts.ref[dat$Type==typeofsite & dat$ref==wtnt & dat$makesCpG==0]
...
#########
# b) All transition mutations from the entire samples
...
filename<-paste0("Output/MutFreq/Fig.ref/Transition_Mut_Freq_Histograms_median.pdf")
pdf(filename, width = 12, height = 7)
par(mfrow=c(2,4))
par(mar = c(4,4.3,2.5,1))
breaks=seq(-6,0,by=.2)
...
datanames<-c("A_syn","T_syn","C_syn","G_syn","A_nonsyn","T_nonsyn","C_nonsyn","G_nonsyn")
colors<-c("#66CCEECC", "#228833CC" ,"#CCBB44CC", "#EE6677CC","#66CCEECC", "#228833CC", "#CCBB44CC", "#EE6677CC")
filename<-paste0("Output/MutFreq/Fig.ref/Transition_Mut_Freq_Histograms_mean.pdf")
pdf(filename, width = 12, height = 7)
par(mfrow=c(2,4))
par(mar = c(4,4.3,2.5,1))
breaks=seq(-6,0,by=.2)
...
filename<-paste0("Output/MutFreq/Fig.ref/Tranversion_Mut_Freq_Histograms_mean.pdf")
pdf(filename, width = 12, height = 7)
par(mfrow=c(2,4))
par(mar = c(4,4.3,2.5,1))
breaks=seq(-6,0,by=.2)
...
mean(log10(datavector1), na.rm = TRUE)


filename<-paste0("Output/MutFreq/Fig.ref/Tranversion_Mut_Freq_Histograms_median.pdf")
pdf(filename, width = 12, height = 7)
par(mfrow=c(2,4))
par(mar = c(4,4.3,2.5,1))
breaks=seq(-6,0,by=.2)
for (i in 1:8){
        ...
        hist(log10(datavector1), breaks = breaks, xlim=c(-5,-0),col=colors[i], 
             main = paste0(nt, " : ", typ), ylab="Count", xlab="",
             xaxt="n", cex.main=1.7, las=1, cex.axis =1,cex.lab=1.5)
        abline(v=log10(median(datavector1, na.rm = TRUE)),col="white",lwd=2,lty=1)
        abline(v=log10(median(datavector1, na.rm = TRUE)),lwd=2,lty=2,col='red')
        x=seq(-5,0,by=1)
        labels <- sapply(x,function(y)as.expression(bquote(10^ .(y))))
        axis(1, at=x,labels=labels, col.axis="black", line=-.7, cex.axis=1)
        mtext(text = "Transversion Mutation Frequency", side = 1, line = 1.5, cex=0.9)
