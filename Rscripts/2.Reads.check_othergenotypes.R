...
#read sam file
HCVsams<-list.files(paste0("Output",geno,"/sam/"),recursive = F,pattern="sam")
...
for (i in 11:length(HCVsams)){

        print(i)
        print(HCVsams[i])
        sam<-read.table(paste0("Output",geno,"/sam/",HCVsams[i]),skip=3, col.names=coln, sep = "\t",fill=T, comment.char="",quote= "")
        sam<-sam[,1:11]
        ...
        consensus<-read.dna(paste0("Output",geno,"/Consensus/",file.name,"_consensus.fasta"), format = "fasta",as.character=TRUE)
...
for (i in 1:length(HCVsams)){
        sam<-read.table(paste("Output",geno,"/sam/",HCVsams[i],sep=""),skip=3, col.names=coln, fill=T)
        sam<-sam[,1:11]
        #only the mapped sequences
        sam<-subset(sam, MapQ>0&MapQ<61)
        sam$seq<-as.character(sam$seq)
...
write.csv(output.summary,paste0("Output",geno,"/HammingDistanceFilteringSummary.csv"))
