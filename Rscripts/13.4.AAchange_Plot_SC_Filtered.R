...
for (i in 1:length(HCVFiles_overview3)){ 
        ...
        Overview_fil[[i]]<-overviews
        names(Overview_fil)[i]<-substr(paste(HCVFiles_overview3[i]),start=1,stop=7)
}
...
filedate<-"2018-09-26"
TsSC<-read.csv(paste0("Output/SelCoeff/FilteredSummary_Transition_SelCoeff_",filedate,".csv"))
TsSC<-TsSC[,-1]
t1<-TsSC
...
table(t1$NofNA)
#eliminate the rows with over 50% NA
t1<-t1[which(t1$NofNA<0.5*s),]

t1$mean<-apply(t1[2:(s+1)], 1, mean, na.rm=T)
range(t1$mean, na.rm=T) #[1]9.470234e-05 9.766501e-01
...
TsSC2<-TsSC[-c(genes$start[4]:genes$end[4]),]

t1<-TsSC2
t1$mean<-apply(t1[2:(s+1)], 1, mean, na.rm=T)
range(t1$mean, na.rm=T) #7.188932e-05 3.440945e-02
...
pchs <- c(16, 17)
ylimval <- -4
pdf(paste0("Output/SummaryFig.Filtered/AAchanges_transition_SCnoHVR1.pdf"), height = 5, width = 9)
par(mar = c(5, 4, 1, 5))
plot(0, type = "n", xlim = c(1, nrow(allComps)), ylim = c(ylimval, 0), axes = FALSE, ylab = "Average mutation frequency", xlab = "Mutation")
axis(side = 2, at = seq(0,ylimval, by=-1), labels = expression(10^0,10^-1, 10^-2, 10^-3, 10^-4), las = 2)
labsize <- .9
