...
bamfiles<-list.files(paste0("Output",geno,"/bam2/"),pattern="bam$")

for (i in 1:length(bamfiles)){
        bam<-bamfiles[i]
        index<-paste0(paste0("Output",geno,"/bam2/",bam),'.bai')
        bf<-BamFile(paste0("Output",geno,"/bam2/",bam), index=index)
        ...
        print(file.name)
        write.csv(summary, file=paste0("Output",geno,"/CSV/",file.name,".csv",collapse=""))
