...
FstFiles<-list.files("Output_all/Diversity/",pattern="^Fst")
...
for (i in 1:length(FstFiles)){ 
        ...
        File<-File[,-1]
        FstList[[i]]<-File
        names(FstList)[i]<-substr(paste(FstFiles[i]),start=1,stop=6)
}
...
for (i in 1:length(FstList2)){
        ...
        title<-paste0("Genotype ",substr(names(FstList2)[i], start=5, stop = 6))
        p1<-ggplot(data=dat, aes(Sample1,Sample2, fill=Fst)) +geom_tile(color="gray40")+ 
                scale_fill_gradient2(low="white", high="blue",midpoint=0.6,limit=c(0,1))+
                theme_light()+
