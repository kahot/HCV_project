...
dir.create("Output3A/Overview_D/")
...
for (f in 1:3){
        ...
        for (i in 1:length(flist1)){ 
                overview<-read.csv(paste0("Output",geno[f],"/Overview3/",flist1[i]),stringsAsFactors=FALSE, row.names=1)
                ...
                filename<-substr(paste(flist1[i]),start=1,stop=7)
                ...
                print(filename)
                write.csv(dat,paste0("Output",geno[f],"/Overview_D/",filename,"_overviewD.csv"))
        }
        ...
        assign(piname, pi)
        write.csv(pi, paste0("Output_all/Diversity/NucDiversity.per.sample.",geno[f],".csv"))
        
        
        pdf(paste0("Output_all/Diversity/Pi_",geno[f],".pdf"), height = 5, width=length(flist1)*0.0192+6.1973)
...
for (g in 1:3){
        ...
        pi<-read.csv(paste0("Output_all/Diversity/NucDiversity.per.sample.",geno[g],".csv"), stringsAsFactors = F, row.names = 1)
        ...
        assign(tname,FstDF)
        write.csv(FstDF,paste0("Output_all/Diversity/Fst_",geno[g],".csv"))
}
