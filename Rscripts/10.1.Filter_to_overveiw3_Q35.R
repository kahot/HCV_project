...
HCVFiles_overview2<-list.files("Output/Overview2/",pattern="overview2.csv")
Overview_sum_ref<-list()
for (i in 1:length(HCVFiles_overview2)){ 
        overviews<-read.csv(paste0("Output/Overview2/",HCVFiles_overview2[i]),stringsAsFactors=FALSE)
...
for (i in 1:length(Overview_sum_ref)){
        ...
        #write.csv(dat,paste0("Output/Overview3/",filename,"_overview3.csv"))
...
write.csv(TMutFreq, "Output/MutFreqQ35/TsMutFreqSummary_Q35.all.csv")

### Calculate mean transition mut. freq
mean(rowMeans(TMutFreq[2:196],na.rm=T),na.rm=T)  #0.005135379
...
write.csv(Ts2,paste0("Output/MutFreqQ35/Filetered.Summary.Ts.Q35.csv"))
