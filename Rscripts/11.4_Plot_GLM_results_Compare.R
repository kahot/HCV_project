...
glmData<-read.csv("Output1A/GLM/GlmData_Ts.csv")
glmData<-glmData[,-1]

for (i in 1:3){
        #d<-read.csv("Output1A/MutFreq.filtered/Filtered.Ts.Q35.csv")
        
        d<-read.csv(paste0("Output1A/Mut.freq.filtered/Summary_",filename[i],".csv"))
        d<-d[which(d$pos==342):nrow(d),-1]

       
        #specify the modcoeff dataframe:
        modcoef<-read.csv(paste0("Output1A/GLM/BetaReg_mod.g2_",filename[i],".csv"),stringsAsFactors = F)
        ...
        pdf(paste0("Output1A/GLM/Mod2_",filename[i],".pdf"),width=12,height=7.5)
