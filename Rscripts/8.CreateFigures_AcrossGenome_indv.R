...
#A) Tests whether nonsyn vs. syn vs. nonsense mutations,& CpGmaking vs. non-CpGmaking muts have different frequencies 

#B) Make SingleSiteFrequencySpectraPRO_58.pdf

# 1). Run tests and make figures for transition mutations
# 2). Make figures only for transition mutations
# 3). Make figures only for sel.coeff calculated using mu from HIV (Abram) (transitions only)
# 4). *very bottom Run tests only
...
###### 1) ########
## Run Tests and Create figures

for (j in 1:length(HCVFiles_overview)){   

for (j in 1:5){   
        id<-names(Overview_summary[j])
        print(id)
        OverviewDF<-Overview_summary[[j]]

        #Test whether non syn muts, syn muts and nonsense muts are different in freq
        FreqsSyn<-OverviewDF$freq.Ts[OverviewDF$Type=="syn"]
        FreqsNonSyn<-OverviewDF$freq.Ts[OverviewDF$Type=="nonsyn"]
        FreqsStop<-OverviewDF$freq.Ts[OverviewDF$Type=="stop"]
        ...
        #Test if CpG making transition mutation frequencies are lower than non-CpGmaking 
        FreqsSyn.CpG<-OverviewDF$freq.Ts[OverviewDF$Type=="syn"&OverviewDF$makesCpG==1]
        FreqsSyn.nonCpG<-OverviewDF$freq.Ts[OverviewDF$Type=="syn"&OverviewDF$makesCpG==0]
        FreqsNonSyn.CpG<-OverviewDF$freq.Ts[OverviewDF$Type=="nonsyn"&OverviewDF$makesCpG==1]
        FreqsNonSyn.nonCpG<-OverviewDF$freq.Ts[OverviewDF$Type=="nonsyn"&OverviewDF$makesCpG==0]
        ...
        #Test if all CpG making mutation frequencies (trans+transv) are lower than non-CpGmaking 
        FreqsSyn.CpG2<-(OverviewDF$freq.Ts[OverviewDF$Type=="syn"&OverviewDF$makesCpG_all==1])+(OverviewDF$freq.transv[OverviewDF$Type=="syn"&OverviewDF$makesCpG_all==1])
        FreqsSyn.nonCpG2<-OverviewDF$freq.Ts[OverviewDF$Type=="syn"&OverviewDF$makesCpG_all==0]+(OverviewDF$freq.transv[OverviewDF$Type=="syn"&OverviewDF$makesCpG_all==0])
        FreqsNonSyn.CpG2<-OverviewDF$freq.Ts[OverviewDF$Type=="nonsyn"&OverviewDF$makesCpG_all==1]+OverviewDF$freq.transv[OverviewDF$Type=="nonsyn"&OverviewDF$makesCpG_all==1]
        FreqsNonSyn.nonCpG2<-OverviewDF$freq.Ts[OverviewDF$Type=="nonsyn"&OverviewDF$makesCpG_all==0]+OverviewDF$freq.transv[OverviewDF$Type=="nonsyn"&OverviewDF$makesCpG_all==0]
        
        print(wilcox.test(FreqsSyn.CpG2, FreqsSyn.nonCpG, alternative = "greater", paired = FALSE))
        print(wilcox.test(FreqsNonSyn.CpG2, FreqsNonSyn.nonCpG, alternative = "greater", paired = FALSE))                
        
        
        #Test whether synonymous C to T  muts and G to A , are more costly than A to G 
        CostSynNoCpGA2G<-as.numeric(OverviewDF$EstSelCoeff[OverviewDF$Type=="syn"&OverviewDF$MajNt=="a"&OverviewDF$makesCpG==0])
        CostSynNoCpGC2T<-as.numeric(OverviewDF$EstSelCoeff[OverviewDF$Type=="syn"&OverviewDF$MajNt=="c"&OverviewDF$makesCpG==0])
        CostSynNoCpGG2A<-as.numeric(OverviewDF$EstSelCoeff[OverviewDF$Type=="syn"&OverviewDF$MajNt=="g"&OverviewDF$makesCpG==0])
        ...
        CostnonSynNoCpGA2G<-as.numeric(OverviewDF$EstSelCoeff[OverviewDF$Type=="nonsyn"&OverviewDF$MajNt=="a"&OverviewDF$makesCpG==0])
        CostnonSynNoCpGC2T<-as.numeric(OverviewDF$EstSelCoeff[OverviewDF$Type=="nonsyn"&OverviewDF$MajNt=="c"&OverviewDF$makesCpG==0])
        CostnonSynNoCpGG2A<-as.numeric(OverviewDF$EstSelCoeff[OverviewDF$Type=="nonsyn"&OverviewDF$MajNt=="g"&OverviewDF$makesCpG==0])
        ...
        #Make a figure with the selection coefficients across genome
                pdf(paste0("Output/EstSelCoeff.Ts_HCV_",id,".pdf"),width=15,height=7.5)
                par(mfrow=c(1,1))
                maxnuc=8500
                par(mar = c(3,5,1,2))
        ...
        for (i in 1:maxnuc){
                ...
                if (OverviewDF$Type[i]=="stop"&OverviewDF$MajNt[i]%in%c("g","c")) {c=1;p=21}
                if (OverviewDF$Type[i]=="syn"&OverviewDF$MajNt[i]%in%c("g","c")) {c=cols[3];p=21}
                if (OverviewDF$Type[i]=="syn"&OverviewDF$MajNt[i]%in%c("a","t")) {c=cols[3];p=21}
                if (OverviewDF$Type[i]=="nonsyn"&OverviewDF$MajNt[i]%in%c("c","g")) {c=cols[4];p=21}
                if (OverviewDF$Type[i]=="nonsyn"&OverviewDF$MajNt[i]%in%c("a","t")) {c=cols[5];p=21}
...
for (j in 1:length(HCVFiles)){ 
        ...
        id<-substr(paste(HCVFiles[j]),start=1,stop=7)
        print(id)
        OverviewDF<-Overview_summary[[j]]
        
        #Make a figure with the selection coefficients across genome
        pdf(paste0("Output/EstSelCoeff_HCV2_",id,".pdf"),width=15,height=7.5)
        par(mfrow=c(1,1))
        maxnuc=8500
        par(mar = c(3,5,1,2))
        selcoeffcolumn = which(names(OverviewDF)=="EstSelCoeff")
...
#####  3) ######
# 3) use Selection Coefficient estimated using HIV mutation rates
...
##############
##############
# Read the transition mutation frequencies file with the positions
freq_Ts<-read.csv("Output/HCV_TsMuts.all.csv")



Overview<-Overview_summary[[1]]
mutation.freq<-Overview$freq.maj[Overview$Type=="syn"]
mutation.freq<-mutation.freq[!is.na(mutation.freq)]
hist(mutation.freq, breaks=)
...
# need List"Overview_summary" from 5.
...
for (j in 1:length(HCVFiles)){  
                ...
                #Test whether synonymous C to T  muts and G to A , are more costly than A to G 
                CostSynNoCpGA2G<-OverviewDF$EstSelCoeff[OverviewDF$Type=="syn"&OverviewDF$MajNt=="a"&OverviewDF$makesCpG==0]
                CostSynNoCpGC2T<-OverviewDF$EstSelCoeff[OverviewDF$Type=="syn"&OverviewDF$MajNt=="c"&OverviewDF$makesCpG==0]
                CostSynNoCpGG2A<-OverviewDF$EstSelCoeff[OverviewDF$Type=="syn"&OverviewDF$MajNt=="g"&OverviewDF$makesCpG==0]
                print(wilcox.test(CostSynNoCpGC2T,CostSynNoCpGA2G,alternative = "greater", paired = FALSE))
                print(wilcox.test(CostSynNoCpGG2A,CostSynNoCpGA2G,alternative = "greater", paired = FALSE))
                
                #Test whether non-synonymous C to T  muts and G to A , are more costly than A to G 
                CostnonSynNoCpGA2G<-OverviewDF$EstSelCoeff[OverviewDF$Type=="nonsyn"&OverviewDF$MajNt=="a"&OverviewDF$makesCpG==0]
                CostnonSynNoCpGC2T<-OverviewDF$EstSelCoeff[OverviewDF$Type=="nonsyn"&OverviewDF$MajNt=="c"&OverviewDF$makesCpG==0]
                CostnonSynNoCpGG2A<-OverviewDF$EstSelCoeff[OverviewDF$Type=="nonsyn"&OverviewDF$MajNt=="g"&OverviewDF$makesCpG==0]
                print(wilcox.test(CostnonSynNoCpGC2T,CostnonSynNoCpGA2G,alternative = "greater", paired = FALSE))
                print(wilcox.test(CostnonSynNoCpGG2A,CostnonSynNoCpGA2G,alternative = "greater", paired = FALSE))
...
for (j in 1:length(HCVFiles)){ 
        ...
        for (i in 1:nrow(OverviewDF)){ 
                if (is.na(OverviewDF$Type.tv1[i])|is.na(OverviewDF$Type.tv2[j])) next
        ...
        for (i in 1:maxnuc){
                ...
                if (OverviewDF$Type.tvs[i]=="nonsyn"&OverviewDF$MajNt[i]%in%c("c","g")) {c=col80[4];p=21}
                if (OverviewDF$Type.tvs[i]=="nonsyn"&OverviewDF$MajNt[i]%in%c("a","t")) {c=col80[5];p=21}
