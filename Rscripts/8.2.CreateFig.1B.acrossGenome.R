...
#Script to create minor variant frequency figure across the genome of 3A (before filtering the mutation frequency). 

#### Prep data
HCVFiles_overview<-list.files("Output1B/Overview2/",pattern="overview2.csv")

#### Skip to "##### read the saved csv files" once run ####
Overview_summary<-list()
for (i in 1:length(HCVFiles_overview)){ 
        overviews<-read.csv(paste0("Output1B/Overview2/",HCVFiles_overview[i]),stringsAsFactors=FALSE)
...
mean(rowMeans(TsMutFreq[2:(s+1)],na.rm=T),na.rm=T)   # 1A 0.008879231   1B:0.00772859    3A: 0.006822925
mean(rowMeans(Tvs.MutFreq[2:(s+1)],na.rm=T),na.rm=T) # 1A:0.001690682   1B:0.001398269   3A: 0.00125646
mean(rowMeans(AllMutFreq[2:(s+1)],na.rm=T),na.rm=T)  # 1A:0.01056991    1B:0.009126858   3A: 0.008079385

# Write csv files
write.csv(TsMutFreq,  "Output1B/MutFreq/TsMutFreq_maj_summary.1B.csv")
write.csv(Tvs.MutFreq,"Output1B/MutFreq/TvsMutFreq_maj_summary.1B.csv")
write.csv(AllMutFreq, "Output1B/MutFreq/AllMutFreq_maj_summary.1B.csv")


#############################
##### read the saved csv files
TsMutFreq <- read.csv("Output1B/MutFreq/TsMutFreq_maj_summary.1B.csv",stringsAsFactors = F)
Tvs.MutFreq<-read.csv("Output1B/MutFreq/TvsMutFreq_maj_summary.1B.csv",stringsAsFactors = F)
AllMutFreq <-read.csv("Output1B/MutFreq/AllMutFreq_maj_summary.1B.csv",stringsAsFactors = F)
...
genes<-read.csv("Data/HCV_annotations1B.csv",stringsAsFactors = F)

MFsummary<-data.frame("Mean"=matrix(nrow=3))
rownames(MFsummary)<-c("AllMF","Transition","Transversion")
for (i in 1:3){
        if (i==1) {SNVFreq<-AllMutFreq; title<-'Average Total Mutation Freq (1B)';name<-"Total_Mutations_1B";yax<-"Average mutation frequency"; ylow<-0.0001;yhigh<-1}
        if (i==2) {SNVFreq<-TsMutFreq; title<-'Average Transition Mutation Freq (1B)';name<-"Transition_1B";yax<- "Average transition mutation frequency";ylow<-0.0001;yhigh<-1}
        if (i==3) {SNVFreq<-Tvs.MutFreq; title<-'Average Transversion Mutation Freq (1B)';name<-"Transversion_1B";yax<- "Average transversion mutation frequency";ylow<-0.00001;yhigh<-0.1}        
        
        #SNVFreq$mean<-apply(SNVFreq[2:(s+1)], 1, mean, na.rm=T)
         
        #Count the number of NAs per row
        SNVFreq$NofNA<-apply(SNVFreq[2:(s+1)], 1, function(x)sum(is.na(x)))  #8537 rows
        
        #2 Filter out sites with over 50% NAs
        SNVFreq1<-SNVFreq[which(SNVFreq$NofNA<0.5*s),]
        print(nrow(SNVFreq1))  #8328 (All)
        SNVFreq1$mean<-apply(SNVFreq1[2:(s+1)], 1, mean, na.rm=T)
        
        
        #2. Filter out sites with only one data points rows
        SNVFreq2<-SNVFreq[-which(SNVFreq$NofNA==(s-1)|SNVFreq$NofNA==s),]  #8370(All)
        SNVFreq2$mean<-apply(SNVFreq2[2:(s+1)], 1, mean, na.rm=T)
        
        
        DFmean<-SNVFreq1[,c("pos","mean")]
        write.csv(DFmean, paste0("Output1B/MutFreq/Ave.MF.",name,'.csv'))
        
        
        startnuc<-SNVFreq1$pos[1]
        endnuc<-SNVFreq1$pos[nrow(SNVFreq1)] #8630
        #endnuc<-SNVFreq2$pos[nrow(SNVFreq2)] #8671
        ...
        pdf(paste0("Output1B/Figures/",title,".pdf"),width=15,height=7.5)
        ...
        points(mean~pos, data=SNVFreqs,pch=20,col=paste0(cols2[2],99),cex=0.5)
        
        #add rolling average
        roll100<-rollmean(SNVFreqs$mean, k=100, na.rm=T)
        SNVFreqs$roll100<-c(rep(NA, times=50),roll100,c(rep(NA, times=49)))
        lines(roll100~pos,data=SNVFreqs, col="#B60043",lwd=1.5)
        ...
        for (j in 1:(nrow(genes)-1)){
                ...
                if (j==1){
                        ...
                        text(xleft+95,1.2*ylow,paste0(genes$Gene[j]),col="black", cex=0.8)
...
write.csv(MFsummary,"Output1B/SummaryStats/MutFreqSummary_Table1B.csv")
...
### The following plots should be made with filtered data (below are with Maj data) ####

########################################
### Plot mutation freq. across the genome based on the mutation types 
## Consider only the sites with Ref = MajNt to simplify

pos<-data.frame(pos=Overview_summary[[1]][1])
Overview_summary2<-list()
for (i in 1:length(HCVFiles_overview)){ 
        overviews<-read.csv(paste0("Output3A/Overview2/",HCVFiles_overview[i]),stringsAsFactors=FALSE)
        overviews<-overviews[,-1]
        overviews<-overviews[overviews$MajNt==overviews$ref,]
        overviews2<-merge(pos, overviews, by="pos",all.x=T)
        Overview_summary2[[i]]<-overviews2
        names(Overview_summary2)[i]<-substr(paste(HCVFiles_overview[i]),start=1,stop=7)
}
...
for (i in 1:length(Overview_summary2)){
        dat<-Overview_summary2[[i]]
        filename<-names(Overview_summary2)[i]
        
        MutFreq_Ts2[[i]]<-dat[,c("pos","freq.Ts")] 
        MutFreq_tv1[[i]]<-dat[,c("pos","freq.transv1")] 
        MutFreq_tv2[[i]]<-dat[,c("pos","freq.transv2")] 
        MutFreq_tvs2[[i]]<-dat[,c("pos","freq.transv")] 
        MutFreq_all2[[i]]<-dat[,c("pos","freq.mutations")] 
        
        names(MutFreq_Ts2)[i]<-filename
        names(MutFreq_tv1)[i]<-filename
        names(MutFreq_tv2)[i]<-filename
        names(MutFreq_tvs2)[i]<-filename
        names(MutFreq_all2)[i]<-filename
        
        
}
#assign column names for the list
for (i in 1:length(MutFreq_Ts)) {
        colnames(MutFreq_Ts2[[i]])<-c("pos",paste0(names(MutFreq_Ts2[i])))
        colnames(MutFreq_tv1[[i]])<-c("pos",paste0(names(MutFreq_tv1[i])))
        colnames(MutFreq_tv2[[i]])<-c("pos",paste0(names(MutFreq_tv2[i])))
        colnames(MutFreq_tvs2[[i]])<-c("pos",paste0(names(MutFreq_tvs2[i])))
        colnames(MutFreq_all2[[i]])<-c("pos",paste0(names(MutFreq_all2[i])))
}

TsMutFreq2<-MutFreq_Ts2%>% purrr::reduce(full_join, by='pos')
Tv1.MutFreq<-MutFreq_tv1 %>% purrr::reduce(full_join, by='pos')
Tv2.MutFreq<-MutFreq_tv2 %>% purrr::reduce(full_join, by='pos')
Tvs.MutFreq2<-MutFreq_tvs2 %>% purrr::reduce(full_join, by='pos')
AllMutFreq2<-MutFreq_all2 %>% purrr::reduce(full_join, by='pos')


s<-length(Overview_summary2)

mean(rowMeans(TsMutFreq2[2:(s+1)],na.rm=T),na.rm=T) #3A: 0.006686469 (removed maj!=ref) 3A: 0.006822925 (all)



dat<-Overview_summary[[3]]
mutationtypes<-dat[,c("pos","ref","Type","Type.tv1","Type.tv2")]
mutationtypes<-mutationtypes[!is.na(mutationtypes$Type),]
...
t2<-t1[which(t1$NofNA<0.5*s),] #7918
t2$mean<-apply(t2[2:(s+1)], 1, mean, na.rm=T)
#relaxed filtering (remove sites with only one data point)
#t2<-t1[-which(t1$NofNA==(s-1)|t1$NofNA==s),] #8366

startpos<-t2$pos[1]
maxpos<-t2$pos[nrow(t2)]

mfs<-merge(mutationtypes,t2,by='pos')
n<-data.frame("pos"=c(1:maxpos))
MF3<-merge(n,mfs,by="pos",all.x=T)

pdf(paste0("Output3A/Figures/Ts_MutFreq_Mutation_Types.3A.pdf"),width=15,height=7.5)
maxnuc=maxpos
par(mar = c(3,5,1,2))
...
for (j in 1:(nrow(genes)-1)){
        ...
        if (j==1){
                ...
                text(xleft+95,1.1*ylow,paste0(genes$Gene[j]),col="black", cex=0.8)
...
tv1.1$mean<-apply(tv1.1[2:(s+1)], 1, mean, na.rm=T)
tv2.1$mean<-apply(tv2.1[2:(s+1)], 1, mean, na.rm=T)
...
pdf(paste0("Output3A/Figures/Transv_MutFreq_Mutation_Types_separate.3A.pdf"),width=15,height=7.5)
maxnuc=8600
par(mar = c(3,5,1,2))
