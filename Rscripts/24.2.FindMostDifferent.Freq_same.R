...
for (g in 1:3){
        ...
        Freq.by.gene<-as.data.frame(cbind(table(mf.H$gene),table(mf.L$gene)))
        colnames(Freq.by.gene)<-c(paste0("higher in ",g1), paste0("higher in ",g2))
        write.csv(Freq.by.gene, paste0("Output_all/Difference/Diff.mf.by.gene.",compare,".csv"))
        Freq.by.gene$Gene<-rownames(Freq.by.gene)
