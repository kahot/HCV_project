...
for (i in 1:length(HCVFiles_SeqData)){   
        ...
        for (codon in 1:(nrow(OverviewDF)/3)) {#for each codon in the sequence
                ...
                if (is.na(Majcodon[1])|is.na(Majcodon[2])|is.na(Majcodon[3])){ 
                        ...
                        mutant1codon.mj <- c(transition(Majcodon[1]), Majcodon[2:3])  
                        mutant2codon.mj <- c(Majcodon[1],transition(Majcodon[2]), Majcodon[3])
                        mutant3codon.mj <- c(Majcodon[1:2], transition(Majcodon[3]))
                ...
                TypeOfSite.maj<-c(TypeOfSite,typeofsitefunction(Majcodon,mutant1codon.mj))
                TypeOfSite.maj<-c(TypeOfSite,typeofsitefunction(Majcodon,mutant2codon.mj))
                TypeOfSite.maj<-c(TypeOfSite,typeofsitefunction(Majcodon,mutant3codon.mj))
        ...
        OverviewDF$Type.maj<-TypeOfSite.maj[1:length(OverviewDF$pos)]
...
for (i in 1:length(Overview)){
        ...
        write.csv(OverviewDF,paste0("Output",geno,"/Overview2/",id,"overview2.csv"))
...
write.csv(ReadsSummary,paste0("Output",geno,"/SeqData/ReadsSummary_",geno,".csv"))      
