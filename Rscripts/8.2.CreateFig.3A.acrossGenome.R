...
TsMutFreq<-read.csv("Output3A/MutFreq/TsMutFreq_maj_summary.3A.csv",stringsAsFactors = F)
Tvs.MutFreq<-read.csv("Output3A/MutFreq/TvsMutFreq_maj_summary.3A.csv",stringsAsFactors = F)
AllMutFreq<-read.csv("Output3A/MutFreq/AllMutFreq_maj_summary.3A.csv",stringsAsFactors = F)
...
genes<-read.csv("Data/HCV_annotations3A.csv",stringsAsFactors = F)

MFsummary<-data.frame("Mean"=matrix(nrow=3))
rownames(MFsummary)<-c("AllMF","Transition","Transversion")
for (i in 1:3){
        if (i==1) {SNVFreq<-AllMutFreq; title<-'Average Total Mutation Freq (3A)';name<-"Total_Mutations_3A";yax<-"Average mutation frequency"; ylow<-0.0001;yhigh<-1}
        if (i==2) {SNVFreq<-TsMutFreq; title<-'Average Transition Mutation Freq (3A)';name<-"Transition_3A";yax<- "Average transition mutation frequency";ylow<-0.0001;yhigh<-1}
        if (i==3) {SNVFreq<-Tvs.MutFreq; title<-'Average Transversion Mutation Freq (3A)';name<-"Transversion_3A";yax<- "Average transversion mutation frequency";ylow<-0.00001;yhigh<-0.1}        
        
        #SNVFreq$mean<-apply(SNVFreq[2:(s+1)], 1, mean, na.rm=T)
         
        #Count the number of NAs per row
        SNVFreq$NofNA<-apply(SNVFreq[2:(s+1)], 1, function(x)sum(is.na(x)))  #8539 rows
        
        #2 Filter out sites with over 50% NAs
        SNVFreq1<-SNVFreq[which(SNVFreq$NofNA<0.5*s),]
        print(nrow(SNVFreq1))  #8382 (Ts)
        SNVFreq1$mean<-apply(SNVFreq1[2:(s+1)], 1, mean, na.rm=T)
        
        
        #2. Filter out sites with only one data points rows
        SNVFreq2<-SNVFreq[-which(SNVFreq$NofNA==(s-1)),]  #8410(Ts)
        SNVFreq2$mean<-apply(SNVFreq2[2:(s+1)], 1, mean, na.rm=T)
        
        startnuc<-SNVFreq1$pos[1]
        endnuc<-SNVFreq1$pos[nrow(SNVFreq1)] #8664
        #endnuc<-SNVFreq2$pos[nrow(SNVFreq2)] #8671
        
        DFmean<-SNVFreq1[,c("pos","mean")]
        write.csv(DFmean, paste0("Output3A/MutFreq/Ave.MF.",name,'.csv'))
        ...
        points(mean~pos, data=SNVFreqs,pch=20,col=paste0(cols2[3],"4D"),cex=0.5)
        
        #add rolling average
        roll100<-rollmean(SNVFreqs$mean, k=100, na.rm=T)
        SNVFreqs$roll100<-c(rep(NA, times=50),roll100,c(rep(NA, times=49)))
        lines(roll100~pos,data=SNVFreqs, col="#0D602B",lwd=1.5)
...
write.csv(MFsummary,"Output3A/SummaryStats/MutFreqSummary_Table3A.csv")
